...
public class Red_Multicapa_Gradiente_Descendiente {
	...
	public Red_Multicapa_Gradiente_Descendiente(double factorAprendizaje){
	...
	//En base a los datos de entrada se procesan unos datos de salida que se cotejan con las salidas esperadas. En base al error entre las salidas reales y las esperadas se corrige 
	// la red neuronal mediante el método de gradiente descendiente.
	public double aprendizaje(double[] datosEntrada,double[] salidasEsperadas){
		//salidasEsperadas = tratamientoDatosEntrada(salidasEsperadas);
		...
		//System.out.println("SALIDA 1:  "+salidasEsperadas[0]+"  SALIDA 2:  "+  salidasEsperadas[1]+"  SALIDA 3:  "+salidasEsperadas[2]+ "   SALIDA 4:  "+salidasEsperadas[3]);
		//System.out.println("SALIDA 1:  "+datosEntrada[3]+"  SALIDA 2:  "+  datosEntrada[4]+"  SALIDA 3:  "+datosEntrada[5]+ "   SALIDA 4:  "+datosEntrada[8]);
		...
		double errorTotal = 0;
		double[] erroresIndividuales = new double[capaSalida.length];
		for(int i=0;i<capaSalida.length;i+=5){
			//erroresIndividuales[i] = Math.abs(salidasEsperadas[i]-salida[i]);
			if(salidasEsperadas[i]!=salida[i] || salidasEsperadas[i+1]!=salida[i+1] || salidasEsperadas[i+2]!=salida[i+2] || salidasEsperadas[i+3]!=salida[i+3] || salidasEsperadas[i+4]!=salida[i+4])
			errorTotal+=1;
		}
		double[] incrementoSalida = new double[capaSalida.length];
		for(int i=0;i<incrementoSalida.length;i++){
			incrementoSalida[i] = 1-Math.pow(salida[i], 2)*errorTotal;
		}
		double errorCapaOculta1 = 0;
		double[] erroresCapaOculta1 = new double[capaOculta1.length];
		for(int i=0;i<capaSalida.length;i++){
			for(int j=0;j<capaSalida[i].peso.length;j++){
				errorCapaOculta1 += incrementoSalida[i]*capaSalida[i].peso[j];
				erroresCapaOculta1[j] = incrementoSalida[i]*capaSalida[i].peso[j];
			}
		}
		double[] incrementoOculta1 = new double[capaOculta1.length];
		for(int i=0;i<incrementoOculta1.length;i++){
			incrementoOculta1[i] = 1-Math.pow(salidaCapaOculta1[i], 2)*erroresCapaOculta1[i];
		}
		for(int i=0;i<capaOculta1.length;i++){
			for(int j=0;j<capaOculta1[i].peso.length;j++){
				capaOculta1[i].peso[j] = capaOculta1[i].peso[j]+(capaOculta1[i].factorAprendizaje*incrementoOculta1[i]*datosEntrada[j]);
			}
		}
		for(int i=0;i<capaSalida.length;i++){
			for(int j=0;j<capaSalida[i].peso.length;j++){
				capaSalida[i].peso[j] = capaSalida[i].peso[j]+(capaSalida[i].factorAprendizaje*incrementoSalida[i]*salidaCapaOculta1[j]);
			}
		}
		return errorTotal;
	}
